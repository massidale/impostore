...
test('Firebase enforces ownership, private data, late joining, atomic cards and disabled Lupus', async () => {
  ...
  await act(host,id,'initIndovinaGame',[{wordSource:'players'}]);await act(host,id,'startIndovinaGame');
  await command(users[4],id,'join',['Late']);
  await Promise.all(users.slice(0,4).map((u,i)=>act(u,id,'submitPlayerWord',[u.uid,`Word ${i}`])));
  ...
  const cards=['Custom A','Custom B'].map(word=>({word,taboo:['a','b','c','d','e']}));
  await act(host,id,'setDictionary',['taboo',cards]);await act(host,id,'startTabooGame');
  ...
  state=await view(describer,id);assert.ok(state.gameState.currentCard.word.startsWith('Custom'));assert.equal(state.gameState.deck,undefined);
